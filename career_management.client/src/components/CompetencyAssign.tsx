...
const CompetencyAssign = () => {
  ...
  const fetchPositions = async () => {
    try {
      const response = await axios.get('https://localhost:7026/api/positioncompetencyrequirements/positions');
  ...
  const fetchCompetencies = async () => {
    try {
      const response = await axios.get('https://localhost:7026/api/competencies');
  ...
  const fetchRequirements = async (positionId: number) => {
    try {
      const response = await axios.get(`https://localhost:7026/api/positioncompetencyrequirements/position/${positionId}`);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to remove this competency requirement?')) {
      ...
      try {
        await axios.delete(`https://localhost:7026/api/positioncompetencyrequirements/${id}?modifiedBy=${currentEmployeeId}`);
        if (selectedPosition) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingRequirement) {
        ...
        await axios.put(`https://localhost:7026/api/positioncompetencyrequirements/${editingRequirement.requirementID}`, {
          ...requirementData,
          requirementID: editingRequirement.requirementID
        });
      } else {
        // Create new requirement
        await axios.post('https://localhost:7026/api/positioncompetencyrequirements', requirementData);
      }
              ...
              {filteredPositions.map((position, index) => (
                <div
                  ...
                  className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors ${
                    selectedPosition?.positionID === position.positionID ? 'bg-blue-50 border-blue-200' : ''
                  }`}
                >
                  <div className="flex justify-between items-start text-left">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        position.assignedCompetenciesCount > 0
              ...
              <div>
                ...
                <div className="relative" ref={competencyDropdownRef}>
                  <button
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent flex items-center justify-between bg-white ${
