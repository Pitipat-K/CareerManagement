...
const CompetencyCategories = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await axios.delete(`https://localhost:7026/api/competencycategories/${id}`);
        fetchCategories();
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCategory) {
        ...
        await axios.put(`https://localhost:7026/api/competencycategories/${editingCategory.categoryID}`, {
          ...categoryData,
          categoryID: editingCategory.categoryID
        });
      } else {
        // Create new category
        await axios.post('https://localhost:7026/api/competencycategories', categoryData);
      }
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    errors.domainID ? 'border-red-500' : 'border-gray-300'
                  }`}
                >
