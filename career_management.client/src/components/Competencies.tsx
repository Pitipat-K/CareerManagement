...
const Competencies = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get('https://localhost:7026/api/competencycategories');
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this competency?')) {
      try {
        await axios.delete(`https://localhost:7026/api/competencies/${id}`);
        fetchCompetencies();
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCompetency) {
        ...
        await axios.put(`https://localhost:7026/api/competencies/${editingCompetency.competencyID}`, {
          ...competencyData,
          competencyID: editingCompetency.competencyID
        });
      } else {
        // Create new competency
        await axios.post('https://localhost:7026/api/competencies', competencyData);
      }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    errors.competencyName ? 'border-red-500' : 'border-gray-300'
                  }`}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    errors.competencyDescription ? 'border-red-500' : 'border-gray-300'
                  }`}
